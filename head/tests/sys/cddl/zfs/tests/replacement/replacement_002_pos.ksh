...
function attach_test
{
	...
	log_must $ZPOOL attach $opt $TESTPOOL $disk1 $disk2
...
for opt in "" "-f"; do
	...
	attach_test "$opt" ${DISKS_A[0]} $replacement_disk
...
for type in "" "raidz" "raidz1"; do
	for op in "" "-f"; do
		...
		log_mustnot $ZPOOL attach "$opt" ${DISKS_A[0]} $replacement_disk

		$ZPOOL iostat -v $TESTPOOL | grep $short_replacement_disk
		if [[ $? -eq 0 ]]; then
			log_fail "$replacement_disk is not present."
		fi
