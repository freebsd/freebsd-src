...
zoned_create(struct gctl_req *req)
{
	...
	if (nzones == 0) {
		gctl_error(req, "Zone size %jd is unusable for %s: it must "
		    "yield at least one zone and at most %u.",
		    (intmax_t)zonesize, dev, G_ZONED_MAXZONES);
		return;
	}
	...
	/*
	 * Hand the geometry to the kernel rather than writing it on the media
	 * and waiting for the taste.  The metadata should be written only once
	 * the device is up: a request the kernel turns down leaves the provider
	 * untouched.
	 */
	zoned_metadata_encode(&md, sector);
	gctl_ro_param(req, "metadata", sizeof(sector), sector);
