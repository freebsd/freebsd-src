...
lAcpiEvaluateObject(lua_State *L)
{
	...
	if (lua_istable(L, 4)) {
		...
		for (int i = 0; i < obj_count; ++i) {
			lua_rawgeti(L, 4, i + 1);
			int elem_idx = lua_gettop(L);

			lua_getfield(L, elem_idx, "Type");
			
			if (ACPI_FAILURE(status = lacpi_int_to_uint32(L, -1,
			    &obj_type))) {
				free_acpi_objs(&objs, i);
				if (pathname != NULL) {
					free(pathname);
					pathname = NULL;
				}
			...
			if (ACPI_FAILURE(status = 
			    build_acpi_obj(L, &objs[i], obj_type, elem_idx))) {
				free_acpi_objs(&objs, i);
	...
	if (return_buffer.Pointer != NULL) {
		...
		lua_pushstring(L, "Success: No object returned");
	}
