...
ACPI_STATUS
lacpi_infer_type(lua_State *L, int idx, UINT32 *type)
{
	if (!lua_istable(L, idx)) {
		return AE_TYPE;
	}

	if (lua_getfield(L, idx, "Integer") != LUA_TNIL) {
		*type = ACPI_TYPE_INTEGER;
	} else if (lua_getfield(L, idx, "String") != LUA_TNIL) {
		*type = ACPI_TYPE_STRING;
	} else if (lua_getfield(L, idx, "Buffer") != LUA_TNIL) {
		*type = ACPI_TYPE_BUFFER;
	} else if (lua_getfield(L, idx, "Elements") != LUA_TNIL) {
		*type = ACPI_TYPE_PACKAGE;
	} else if (lua_getfield(L, idx, "Handle") != LUA_TNIL) {
		*type = ACPI_TYPE_LOCAL_REFERENCE;
	} else if (lua_getfield(L, idx, "ProcID") != LUA_TNIL) {
		*type = ACPI_TYPE_PROCESSOR;
	} else if (lua_getfield(L, idx, "SystemLevel") != LUA_TNIL) {
...
ACPI_STATUS
build_int(lua_State *L, ACPI_OBJECT *obj, int idx)
{
	idx = lua_absindex(L, idx);
	obj->Type = ACPI_TYPE_INTEGER;

	lua_getfield(L, idx, "Integer");
...
ACPI_STATUS
build_str(lua_State *L, ACPI_OBJECT *obj, int idx)
{
	idx = lua_absindex(L, idx);
...
ACPI_STATUS
build_buff(lua_State *L, ACPI_OBJECT *obj, int idx)
{
	idx = lua_absindex(L, idx);
...
ACPI_STATUS
build_pkg(lua_State *L, ACPI_OBJECT *obj, int idx)
{
	idx = lua_absindex(L, idx);
	ACPI_STATUS status;
	obj->Type = ACPI_TYPE_PACKAGE;
	
	lua_getfield(L, idx, "Elements");
	int ele_idx = lua_gettop(L);
	
	obj->Package.Count = lua_rawlen(L, ele_idx);
	...
	for (UINT32 i = 0; i < obj->Package.Count; ++i) {
		lua_rawgeti(L, ele_idx, i + 1);
		int curr_idx = lua_gettop(L);

		lua_getfield(L, curr_idx, "Type");

		UINT32 obj_type;
		if (ACPI_FAILURE(lacpi_int_to_uint32(L, -1, &obj_type))) {
			lua_pop(L, 2);
			free_acpi_objs(&obj->Package.Elements, i);
			return AE_NUMERIC_OVERFLOW;
		}
		lua_pop(L, 1);
		
		if (ACPI_FAILURE(status = build_acpi_obj(L,
		    &obj->Package.Elements[i], obj_type, curr_idx))) {
			lua_pop(L, 1);
			free_acpi_objs(&obj->Package.Elements, i);
			return status;
		}
...
ACPI_STATUS
build_ref(lua_State *L, ACPI_OBJECT *obj, int idx)
{
	idx = lua_absindex(L, idx);
	obj->Type = ACPI_TYPE_LOCAL_REFERENCE;
	
	lua_getfield(L, idx, "ActualType");
	...
	ACPI_HANDLE handle = *(ACPI_HANDLE *)luaL_checkudata(L, -1,
	    "lacpi_node");
...
ACPI_STATUS
build_proc(lua_State *L, ACPI_OBJECT *obj, int idx)
{
	idx = lua_absindex(L, idx);
	obj->Type = ACPI_TYPE_PROCESSOR;

	lua_getfield(L, idx, "ProcID");
	...
	lua_getfield(L, idx, "PblkAddress");
	if (!lua_isinteger(L, -1)) {
		lua_pop(L, 1);
		return AE_TYPE;
	}
...
ACPI_STATUS
build_pow(lua_State *L, ACPI_OBJECT *obj, int idx)
{
	idx = lua_absindex(L, idx);
	obj->Type = ACPI_TYPE_POWER;

	lua_getfield(L, idx, "SystemLevel");
	if (ACPI_FAILURE(lacpi_int_to_uint32(L, -1,
	...
	lua_getfield(L, idx, "ResourceOrder");
	if (ACPI_FAILURE(lacpi_int_to_uint32(L, -1,
...
ACPI_STATUS
build_acpi_obj(lua_State *L, ACPI_OBJECT *obj, const UINT32 obj_type, int idx)
{
	switch(obj_type) {
		case ACPI_TYPE_INTEGER:
			return build_int(L, obj, idx);
		case ACPI_TYPE_STRING:
			return build_str(L, obj, idx);
		case ACPI_TYPE_BUFFER:
			return build_buff(L, obj, idx);
		case ACPI_TYPE_PACKAGE:
			return build_pkg(L, obj, idx);
		case ACPI_TYPE_LOCAL_REFERENCE:
			return build_ref(L, obj, idx);
		case ACPI_TYPE_PROCESSOR:
			return build_proc(L, obj, idx);
		case ACPI_TYPE_POWER:
			return build_pow(L, obj, idx);
		default:
			return AE_ERROR;
	}
}
...
void
free_acpi_objs(ACPI_OBJECT **objs, UINT32 init_count)
{
	if (*objs) {
