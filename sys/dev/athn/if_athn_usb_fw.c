...
athn_usb_load_firmware(struct athn_usb_softc *usc)
{
	...
	const struct firmware *fw, *ptr;
	size_t fwsize, size;
	uint32_t addr;
	int s, mlen;
	int error = ENXIO;

	fw = athn_usb_get_firmware(usc);
	...
	while (size > 0) {
		...
		error = usbd_do_request(usc->sc_udev, &usc->sc_mtx, &req, (void*)ptr);
		if (error != 0) {
...
//	free(fw, M_DEVBUF, fwsize);
	...
	error = usbd_do_request(usc->sc_udev, &usc->sc_mtx, &req, NULL);
