...
trap "rm $sysaue $sysdcl $syscompat $syscompatdcl $syscompat4 $syscompat4dcl $syscompat6 $syscompat6dcl $syscompat7 $syscompat7dcl $sysent $sysinc $sysarg $sysprotoend $systracetmp $systraceret $sysstubfwd $sysstubstubs" 0

touch $sysaue $sysdcl $syscompat $syscompatdcl $syscompat4 $syscompat4dcl $syscompat6 $syscompat6dcl $syscompat7 $syscompat7dcl $sysent $sysinc $sysarg $sysprotoend $systracetmp $systraceret $sysstubfwd $sysstubstubs
...
#printf("in %s, var %s has scaler dep %s\n", funcalias, argname[i], dep)
		...
		if (!flag("NOSTUB") && !flag("NODEF")) {
			if (!flag("VARARG")) {
				...
				printf ("SYS_STUB(%s, %s, %s, (", syscall, userrettype,
				...
				nocheri_funcname = funcname
				sub(/cheriabi_/, "", nocheri_funcname)
				printf("SYS_STUB_VA(%s, %s, %s, (", syscall, userrettype,
				    nocheri_funcname) > sysstubstubs
				if (argc == 0) {
					printf("%s tagged VARARG, but has no arguments\n",
					    funcname)
					exit 1
				}

				# Print decl args with ... as last arg
				for (i = 1; i < argc; i++) {
					if (i == 1)
						comma = ""
					else
				...
				# Print decl args with (va_list ap) as last arg
				for (i = 1; i < argc; i++) {
					if (i == 1)
						comma = ""
					else
				...
				# Print argument names including ap
				for (i = 1; i < argc; i++) {
					if (i == 1)
						comma = ""
					else
