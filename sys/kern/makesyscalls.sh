...
sysargparse="/dev/null"
sysargparse_h="_SYS_SYSARGPARSE_H_"
sysnames="syscalls.c"
...
' < $1 | awk "
	BEGIN {
		...
		sysargparse = \"$sysargparse\"
		sysargparse_h = \"$sysargparse_h\"
		sysnames = \"$sysnames\"
		syshdr = \"$syshdr\"
		sysmk = \"$sysmk\"
		systrace = \"$systrace\"
	...
	type("STD") || type("NODEF") || type("NOARGS") || type("NOPROTO") \
		...
		if (argc != 0 && flag("NOPARSE")) {
			printf("int\t") > sysargparse
			printf("%s%s_parseargs(struct thread *td,\n",
			    syscallprefix, funcalias) > sysargparse
			printf("    struct %s *uap);\n\n",
			    argalias) > sysargparse
		}
		if (argc != 0 && !flag("NOPARSE")) {
...
#printf("in %s, var %s has scaler dep %s\n", funcalias, argname[i], dep)
			...
			if (pointers != 0) {
				...
				printf("\tsize_t base, length, offset;\n") > sysargparse
			}
