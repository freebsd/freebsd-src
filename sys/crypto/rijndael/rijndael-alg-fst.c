...
int rijndaelKeySched(u8 k[RIJNDAEL_MAXKC][4], u8 W[RIJNDAEL_MAXROUNDS+1][4][4], int ROUNDS) {
	/* Calculate the necessary round keys
	 * The number of calculations depends on keyBits and blockBits
	 */ 
	int j, r, t, rconpointer = 0;
	union {
		u8	x8[RIJNDAEL_MAXKC][4];
		u32	x32[RIJNDAEL_MAXKC];
	} xtk;
#define	tk	xtk.x8
	int KC = ROUNDS - 6;
...
int rijndaelKeyEncToDec(u8 W[RIJNDAEL_MAXROUNDS+1][4][4], int ROUNDS) {
	int r;
	u8 *w;
 ...
 */
int rijndaelEncrypt(u8 in[16], u8 out[16], u8 rk[RIJNDAEL_MAXROUNDS+1][4][4], int ROUNDS) {
	int r;
	union {
		u8	x8[16];
		u32	x32[4];
...
#define	temp	xtemp.x8
    ...
    *((u32*)(b    )) = *((const u32*)T1[temp[0][0]])
					^ *((const u32*)T2[temp[1][1]])
					^ *((const u32*)T3[temp[2][2]]) 
					^ *((const u32*)T4[temp[3][3]]);
    *((u32*)(b + 4)) = *((const u32*)T1[temp[1][0]])
					^ *((const u32*)T2[temp[2][1]])
					^ *((const u32*)T3[temp[3][2]]) 
					^ *((const u32*)T4[temp[0][3]]);
    *((u32*)(b + 8)) = *((const u32*)T1[temp[2][0]])
					^ *((const u32*)T2[temp[3][1]])
					^ *((const u32*)T3[temp[0][2]]) 
					^ *((const u32*)T4[temp[1][3]]);
    *((u32*)(b +12)) = *((const u32*)T1[temp[3][0]])
					^ *((const u32*)T2[temp[0][1]])
					^ *((const u32*)T3[temp[1][2]]) 
					^ *((const u32*)T4[temp[2][3]]);
 ...
 */
int rijndaelEncryptRound(u8 a[4][4], u8 rk[RIJNDAEL_MAXROUNDS+1][4][4], int ROUNDS, int rounds) {
	int r;
	u8 temp[4][4];
	...
	*((u32*)a[0]) = *((u32*)a[0]) ^ *((u32*)rk[0][0]);
	*((u32*)a[1]) = *((u32*)a[1]) ^ *((u32*)rk[0][1]);
	*((u32*)a[2]) = *((u32*)a[2]) ^ *((u32*)rk[0][2]);
	*((u32*)a[3]) = *((u32*)a[3]) ^ *((u32*)rk[0][3]);

	for (r = 1; (r <= rounds) && (r < ROUNDS); r++) {
		*((u32*)temp[0]) = *((u32*)T1[a[0][0]])
           ^ *((u32*)T2[a[1][1]])
           ^ *((u32*)T3[a[2][2]]) 
           ^ *((u32*)T4[a[3][3]]);
		*((u32*)temp[1]) = *((u32*)T1[a[1][0]])
           ^ *((u32*)T2[a[2][1]])
           ^ *((u32*)T3[a[3][2]]) 
           ^ *((u32*)T4[a[0][3]]);
		*((u32*)temp[2]) = *((u32*)T1[a[2][0]])
           ^ *((u32*)T2[a[3][1]])
           ^ *((u32*)T3[a[0][2]]) 
           ^ *((u32*)T4[a[1][3]]);
		*((u32*)temp[3]) = *((u32*)T1[a[3][0]])
           ^ *((u32*)T2[a[0][1]])
           ^ *((u32*)T3[a[1][2]]) 
           ^ *((u32*)T4[a[2][3]]);
		*((u32*)a[0]) = *((u32*)temp[0]) ^ *((u32*)rk[r][0]);
		*((u32*)a[1]) = *((u32*)temp[1]) ^ *((u32*)rk[r][1]);
		*((u32*)a[2]) = *((u32*)temp[2]) ^ *((u32*)rk[r][2]);
		*((u32*)a[3]) = *((u32*)temp[3]) ^ *((u32*)rk[r][3]);
	}
	if (rounds == ROUNDS) {
	   	/* last round is special */   
	   	temp[0][0] = T1[a[0][0]][1];
	   	...
	   	temp[3][2] = T1[a[1][2]][1]; 
	   	temp[3][3] = T1[a[2][3]][1];
		*((u32*)a[0]) = *((u32*)temp[0]) ^ *((u32*)rk[ROUNDS][0]);
		*((u32*)a[1]) = *((u32*)temp[1]) ^ *((u32*)rk[ROUNDS][1]);
		*((u32*)a[2]) = *((u32*)temp[2]) ^ *((u32*)rk[ROUNDS][2]);
		*((u32*)a[3]) = *((u32*)temp[3]) ^ *((u32*)rk[ROUNDS][3]);
	}
 ...
 */
int rijndaelDecrypt(u8 in[16], u8 out[16], u8 rk[RIJNDAEL_MAXROUNDS+1][4][4], int ROUNDS) {
	int r;
	union {
		u8	x8[16];
		u32	x32[4];
...
#define	temp	xtemp.x8
    ...
    *((u32*)(b   )) = *((const u32*)T5[temp[0][0]])
           ^ *((const u32*)T6[temp[3][1]])
           ^ *((const u32*)T7[temp[2][2]]) 
           ^ *((const u32*)T8[temp[1][3]]);
	*((u32*)(b+ 4)) = *((const u32*)T5[temp[1][0]])
           ^ *((const u32*)T6[temp[0][1]])
           ^ *((const u32*)T7[temp[3][2]]) 
           ^ *((const u32*)T8[temp[2][3]]);
	*((u32*)(b+ 8)) = *((const u32*)T5[temp[2][0]])
           ^ *((const u32*)T6[temp[1][1]])
           ^ *((const u32*)T7[temp[0][2]]) 
           ^ *((const u32*)T8[temp[3][3]]);
	*((u32*)(b+12)) = *((const u32*)T5[temp[3][0]])
           ^ *((const u32*)T6[temp[2][1]])
           ^ *((const u32*)T7[temp[1][2]]) 
           ^ *((const u32*)T8[temp[0][3]]);
 ...
 * of encryption.
 */
int rijndaelDecryptRound(u8 a[4][4], u8 rk[RIJNDAEL_MAXROUNDS+1][4][4], int ROUNDS, int rounds) {
	int r, i;
	u8 temp[4], shift;
	...
	*(u32 *)a[0] ^= *(u32 *)rk[ROUNDS][0];
	*(u32 *)a[1] ^= *(u32 *)rk[ROUNDS][1];
	*(u32 *)a[2] ^= *(u32 *)rk[ROUNDS][2];
	*(u32 *)a[3] ^= *(u32 *)rk[ROUNDS][3];
	for (i = 0; i < 4; i++) {
		a[i][0] = Si[a[i][0]];
		a[i][1] = Si[a[i][1]];
		a[i][2] = Si[a[i][2]];
