	...
	xfs_off_t	stop)
{
	...
	if (flag & FSYNC_INVAL) {
		if (ip->i_df.if_flags & XFS_IFEXTENTS &&
		    ip->i_df.if_bytes > 0) {
			XVOP_FLUSHINVAL_PAGES(vp, start, -1, FI_REMAPF_LOCKED);
		}
	...
	bhv_desc_t	*bdp)
{
	...
	if ((ip->i_d.di_mode & S_IFMT) == S_IFREG) {
		if (ip->i_d.di_size > 0) {
			 ...
			 */
			if (!XFS_FORCED_SHUTDOWN(ip->i_mount)) {
				XVOP_FLUSHINVAL_PAGES(vp, 0, -1, FI_NONE);
			} else {
				XVOP_TOSS_PAGES(vp, 0, -1, FI_NONE);
			}
