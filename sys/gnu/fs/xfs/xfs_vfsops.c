	...
	cred_t		*credp)
{
	 ...
	 */
	error = xfs_iflush_all(mp, 0) ? 0 : XFS_ERROR(EBUSY);
	if (error)
		goto out;
	...
	struct xfs_mount_args		*args)
{
	...
	if (*flags & MNT_RDONLY) {
		 ...
		 */ 
		do {
			XVFS_SYNC(vfsp, REMOUNT_READONLY_FLAGS, NULL, error);
