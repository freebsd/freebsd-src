    ...
    struct g_provider *pp)
{
	...
	sc->sc_zones = malloc(nzones * sizeof(*sc->sc_zones), M_ZONED,
	    M_NOWAIT | M_ZERO);
	if (sc->sc_zones == NULL) {
		G_ZONED_DEBUG(0, "Cannot allocate a %u zone table for %s.",
		    sc->sc_nzones, pp->name);
		g_free(sc);
		g_destroy_geom(gp);
		return (NULL);
	}
...
static void
g_zoned_ctl_create(struct gctl_req *req, struct g_class *mp)
{
	struct g_zoned_metadata md;
	struct g_provider *pp;
	struct g_consumer *cp;
	...
	sector = gctl_get_param(req, "metadata", &len);
	if (sector == NULL) {
		gctl_error(req, "No '%s' argument.", "metadata");
		return;
	}
	if (len < (int)sizeof(struct g_zoned_metadata) ||
	    len > (int)pp->sectorsize) {
		gctl_error(req, "Invalid '%s' argument.", "metadata");
		return;
	}

	/*
	 * The geometry was measured in userland, before the request reached
	 * the topology lock, so make sure the provider still matches it.
	 */
	zoned_metadata_decode(sector, &md);
	if (memcmp(md.md_magic, G_ZONED_MAGIC, sizeof(G_ZONED_MAGIC)) != 0 ||
	    md.md_version != G_ZONED_VERSION ||
	    md.md_provsize != (uint64_t)pp->mediasize ||
	    md.md_sectorsize != pp->sectorsize) {
		gctl_error(req, "Invalid metadata for %s.", name);
		return;
	}

	gp = g_zoned_create(mp, &md, pp);
	if (gp == NULL) {
		gctl_error(req, "Cannot create a zoned device on %s.", name);
		return;
	}

	/* The tail sector is ours now; pad the metadata out to fill it. */
	cp = LIST_FIRST(&gp->consumer);
	buf = g_malloc(pp->sectorsize, M_WAITOK | M_ZERO);
	memcpy(buf, sector, len);
	g_topology_unlock();
