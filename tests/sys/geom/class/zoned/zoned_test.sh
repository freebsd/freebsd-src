...
atf_test_case create_readonly cleanup
create_readonly_head()
{
	atf_set "descr" "gzoned refuses a provider it cannot write to"
	atf_set "require.user" "root"
}
create_readonly_body()
{
	gzoned_test_setup

	atf_check truncate -s 1025m backing_file
	attach_md md -t vnode -f backing_file -o readonly
	atf_check -s not-exit:0 -e ignore gzoned create -s 256m ${md}
	atf_check test ! -c /dev/${md}.zoned
}
create_readonly_cleanup()
{
	gzoned_test_cleanup
}

atf_test_case zone_size_too_large cleanup
zone_size_too_large_head()
{
	atf_set "descr" "gzoned refuses a zone size larger than the provider"
	atf_set "require.user" "root"
}
zone_size_too_large_body()
{
	gzoned_test_setup

	zoned_backing_md
	atf_check -s not-exit:0 -e match:"unusable" \
	    gzoned create -s 2g ${md}
	atf_check test ! -c /dev/${md}.zoned
}
zone_size_too_large_cleanup()
{
	gzoned_test_cleanup
}

atf_test_case zone_size_too_many cleanup
zone_size_too_many_head()
{
	atf_set "descr" "gzoned refuses a zone size yielding too many zones"
	atf_set "require.user" "root"
}
zone_size_too_many_body()
{
	gzoned_test_setup

	# One zone per sector on a sparse 8T provider overflows the 32-bit zone
	# count.
	atf_check truncate -s 8t backing_file
	attach_md md -t vnode -f backing_file
	atf_check -s not-exit:0 -e match:"unusable" \
	    gzoned create -s 512 ${md}
	atf_check test ! -c /dev/${md}.zoned
}
zone_size_too_many_cleanup()
{
	gzoned_test_cleanup
}
...
reset_wp_all_readonly_body()
{
	...
	# Read-only zone reports no write pointer.  Clear the fault to see
	# whether the pointer was left where it was beforehand.
...
atf_init_test_cases()
{
	...
	atf_add_test_case create_readonly
	atf_add_test_case zone_size_too_large
	atf_add_test_case zone_size_too_many
