...
/*
 * Class, PMC_F_EV_PMU, and subclass mismatches in amd_allocate_pmc are
 * row discriminators: every row of the requested class will reject the
 * request with a bare EINVAL (no EXTERROR), and the loop in hwpmc_mod
 * exhausts all rows. The user-visible diagnostic is therefore the
 * "No PMC row accepted" message, not a per-row mismatch string.
 */
...
ATF_TC_BODY(ibs_invalid_config_bits, tc)
{
	...
	ATF_REQUIRE_ERRNO(EINVAL, hwpmc_call(PMC_OP_PMCALLOCATE, &pa) == -1);
	require_exterr("Unsupported IBS op control bits");
}
