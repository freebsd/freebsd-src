...
ATF_TC_BODY(pmcrw_no_flags, tc)
{
	...
	require_exterr("PMCRW requires PMC_F_OLDVALUE");
...
ATF_TC_BODY(amd_invalid_config_bits, tc)
{
	...
	ATF_REQUIRE_ERRNO(EINVAL, hwpmc_call(PMC_OP_PMCALLOCATE, &pa) == -1);
	ATF_REQUIRE_EQ(0, uexterr_gettext(exterr, sizeof(exterr)));
	if (strstr(exterr, "No PMC row accepted") != NULL)
		atf_tc_skip("AMD core reserved-bit diagnostic was not returned");
	ATF_REQUIRE_MSG(strstr(exterr,
	    "AMD PMU config has unsupported bits") != NULL,
	    "extended error \"%s\" does not report core reserved-bit failure",
	    exterr);
}

/*
 * DF subclass with reserved config bits. On a system with Data Fabric
 * PMC support, the DF row matches class+subclass and the config-mask
 * check fires with the "AMD PMU config has unsupported bits" diagnostic.
 * On a system without DF support every row mismatches subclass and we
 * surface "No PMC row accepted"; the test skips in that case rather
 * than fail.
 */
ATF_TC_WITHOUT_HEAD(amd_invalid_df_config_bits);
ATF_TC_BODY(amd_invalid_df_config_bits, tc)
{
	struct pmc_op_pmcallocate pa;
	char exterr[UEXTERROR_MAXLEN];
	...
	pa.pm_cpu = 0;
	pa.pm_flags = PMC_F_EV_PMU;
	pa.pm_md.pm_amd.pm_amd_sub_class = PMC_AMD_SUB_CLASS_DATA_FABRIC;
	pa.pm_md.pm_amd.pm_amd_config = ~0ULL;

	ATF_REQUIRE_ERRNO(EINVAL, hwpmc_call(PMC_OP_PMCALLOCATE, &pa) == -1);
	ATF_REQUIRE_EQ(0, uexterr_gettext(exterr, sizeof(exterr)));
	if (strstr(exterr, "No PMC row accepted") != NULL)
		atf_tc_skip("Data Fabric PMCs are unavailable on this system");
	ATF_REQUIRE_MSG(strstr(exterr,
	    "AMD PMU config has unsupported bits") != NULL,
	    "extended error \"%s\" does not report DF reserved-bit failure",
	    exterr);
	fprintf(stderr, "DF reserved-bit allocation: errno=EINVAL exterr=%s\n",
	    exterr);
}
...
ATF_TC_BODY(ibs_missing_system_capability, tc)
{
	...
	pa.pm_cpu = 0;
	pa.pm_md.pm_ibs.ibs_type = IBS_PMC_OP;
...
ATF_TC_BODY(ibs_invalid_config_bits, tc)
{
	...
	pa.pm_cpu = 0;
	pa.pm_md.pm_ibs.ibs_type = IBS_PMC_OP;
	pa.pm_md.pm_ibs.ibs_ctl = 1ULL << 58;

	ATF_REQUIRE_ERRNO(EINVAL, hwpmc_call(PMC_OP_PMCALLOCATE, &pa) == -1);
	require_exterr("IBS op ctl config");
}
...
ATF_TP_ADD_TCS(tp)
{
	...
	ATF_TP_ADD_TC(tp, amd_invalid_df_config_bits);
