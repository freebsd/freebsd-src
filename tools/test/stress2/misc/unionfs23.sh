...
# unionfs(4) test
# Variation of unionfs22.sh, but with '-o below'
# Reproduces "lockmgr_xlock_hard: recursing on non recursive lockmgr"
# panic (PR 298201)
...
mount -t unionfs -o noatime -o below $mp1 $mp3
set +e
...
n=`find $mp3/stressX | wc -l`
[ $n -eq 1 ] && s=0 || { find $mp3/stressX -ls | head -12; s=1; }
umount $mp2
umount $mp1
exit $s
