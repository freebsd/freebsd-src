...
(cd $DESTDIR;
 ...
 find -d -s . -type d -mindepth 1 -and \! -empty | \
    egrep -v '^\./(boot|s*bin|lib|libexec|usr|usr/include|usr/lib(32|data|exec)?|usr/libdata/pkgconfig|usr/lib/private|usr/s*bin|usr/share|usr/share/(examples|man|misc|nls)|usr/share/man/man[0-9])$' | \
    sed -e 's,^,OLD_DIRS+=,'
) | sed -e 's,+=\./,+=,'
