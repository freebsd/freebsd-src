...
CFLAGS="${CFLAGS_COMMON} ${CFLAGS_LIBSTATCOUNTERS} --sysroot=${CHERI_SDK256}/sysroot -B${CHERI_SDK256}/bin -target cheri-unknown-freebsd -mabi=purecap" export CFLAGS
...
CC="${CHERI_SDK256}/bin/clang" export CC
CFLAGS="${CFLAGS_COMMON} ${CFLAGS_LIBSTATCOUNTERS} --sysroot=${CHERI_SDK256}/sysroot -B${CHERI_SDK256}/bin -target cheri-unknown-freebsd -mabi=n64" export CFLAGS
