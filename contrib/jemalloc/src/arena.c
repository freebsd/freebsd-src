...
{
	uintptr_t a_miscelm = (vaddr_t)a;
	uintptr_t b_miscelm = (vaddr_t)b;
...
arena_decay_init(arena_t *arena, ssize_t decay_time)
{
	...
	arena->decay.jitter_state = (uint64_t)(vaddr_t)arena;
	arena_decay_deadline_init(arena);
	arena->decay.ndirty = arena->ndirty;
    ...
    size_t alignment, bool zero, tcache_t *tcache)
{
	...
	if (likely(usize <= large_maxclass)) {
		...
		if (!arena_ralloc_no_move(tsd_tsdn(tsd), ptr, oldsize, usize, 0,
		    zero))
			/* XXX-BD: CHERI reinstall permissions? */
			return (UNBOUND_PTR(ptr));
