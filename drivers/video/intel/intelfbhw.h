...
/* $DHD: intelfb/intelfbhw.h,v 1.3 2003/02/05 20:23:56 dawes Exp $ */
/* $TG$ */
...
/* I/O macros */
#define INREG8(addr)		readb(dinfo->mmio_base + (addr))
#define INREG(addr)		readl(dinfo->mmio_base + (addr))
#define OUTREG8(addr, val)	writeb((val), dinfo->mmio_base + (addr))
#define OUTREG(addr, val)	writel((val), dinfo->mmio_base + (addr))

/* Ring buffer macros */
#define OUT_RING(n)	do {						\
	writel((n), dinfo->ring_base + dinfo->ring_tail);		\
	dinfo->ring_tail += 4;						\
...
#define START_RING(n)	do {						\
	u32 needed;							\
	needed = (n) * 4;						\
	if (dinfo->ring_tail + needed >					\
	    dinfo->ring_tail_mask - RING_MIN_FREE) {			\
		needed += dinfo->ring_tail_mask + 1 - dinfo->ring_tail;	\
	}								\
	if (dinfo->ring_space < needed)					\
		wait_ring(dinfo, needed);				\
	dinfo->ring_space -= needed;					\
	while (needed > (n) * 4) {					\
		OUT_RING(MI_NOOP);					\
		needed -= 4;						\
	}								\
