...
fas216_reselected_intr(FAS216_Info *info)
{
	...
	if (info->scsi.phase == PHASE_SELECTION ||
		...
		fas216_log(info, LOG_ERROR, "wrong phase in reselected_intr\n");
		return;
	}
	...
	if (info->scsi.disconnectable && info->SCpnt) {
		...
		if (info->SCpnt->target == target &&
		    info->SCpnt->lun    == lun &&
		    info->SCpnt->tag    == tag) {
			fas216_log(info, LOG_CONNECT, "reconnected previously connected command");
		} else {
...
#define TYPE_QUEUE	2
	...
	fas216_log_command(info, LOG_CONNECT | LOG_MESSAGES, SCpnt, "starting ");
